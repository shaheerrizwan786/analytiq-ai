...
def _extract_detailed_reviews(
    ...
    *,
    ...
    for item in client.dataset(dataset_id).iterate_items():
        ...
        place_id_row = _to_str(item.get("placeId"))
...
async def _extract_place_url_from_crawler_async(
    settings: Settings,
    *,
    restaurant_name: str,
    restaurant_location: str,
) -> tuple[str | None, str | None, str | None]:
    """Stage 1 async: crawler-google-places via ApifyClientAsync."""
    if not settings.apify_api_key:
        raise GoogleReviewsError("APIFY_API_KEY is not set")
...
async def _extract_detailed_reviews_async(
    settings: Settings,
    *,
    place_url: str,
    max_reviews: int,
    ...
    """Stage 2 async: Google-Maps-Reviews-Scraper via ApifyClientAsync."""
    if not settings.apify_api_key:
        raise GoogleReviewsError("APIFY_API_KEY is not set")
...
async def fetch_google_reviews_detailed_async(
    settings: Settings,
    *,
    restaurant_name: str,
    restaurant_location: str,
    ...
    """Async two-stage Google Maps review extraction (ApifyClientAsync)."""
    max_reviews = settings.apify_google_max_reviews
    ...
    if not place_url:
        place_url, place_id, stage1_dataset_url = await _extract_place_url_from_crawler_async(
            settings,
            restaurant_name=restaurant_name,
            restaurant_location=restaurant_location,
        )
    ...
    reviews, stage2_dataset_url = await _extract_detailed_reviews_async(
        settings,
        place_url=place_url,
        max_reviews=max_reviews,
        since=since,
